...
import { Row, Col, Button, Input, Modal, Card, Divider, Image, message } from 'antd';
...
const UserDetail = (props) => {
    ...
    const changePassWord = async () => {
        try {
            ...
            const { data } = await axios.put(`${API_URL}/user/changepassword`, body, config)
            handleCancel()
            console.log(data);
        } catch (error) {
            console.log(error.response);
                                ...
                                <Row>
                                    <Col span={24}>
                                                ...
                                                <Card style={{ marginBottom: '5px' }}>
                                                    {
                                                        dataUrlMarket.map((market, i) => (
                                                            <React.Fragment key={i}>
                                                                <a style={{ color: 'rgba(0, 0, 0, 0.85)' }} href={market}>{market}</a><br />
                                                            </React.Fragment>
                                                        ))
                                                    }
                                                </Card>
                                        ...
                                        <div className="user-info" style={{ marginTop: 20 }}>
                                            ...
                                            <div>
                                                <Input size="large" defaultValue="" suffix={<EditOutlined onClick={() => onChangePassword()} />} />
                                            </div>
                                        </div>

                                        <Row gutter={24} style={{ border: '1px dashed #cccccc', padding: '1rem', margin: '5px 40px 0 40px' }} justify="center">
                                            ...
                                            <Col xs={24} sm={17} md={17} lg={17} xl={17} style={{ textAlign: "center" }}>
                                        ...
                                        <div className="user-info" style={{ marginTop: 20 }}>
                                            <div className="actions">
                                                <Button size="large" type="" className="btn-save">로그인</Button>
                                            </div>
                                        </div>
                                    </Col>
                                </Row>
                                ...
                                <Modal title="Change Password" visible={isModalVisible} footer={false} onCancel={handleCancel}>
                                    <div style={{ padding: '0 80px' }}>
                                        <div style={{ marginBottom: 20 }}>
                                            <Input.Password name="current_password" onChange={onChange} size="large" placeholder="Your Password" />
                                        </div>
                                        <div>
                                            <Input.Password name="new_password" onChange={onChange} size="large" placeholder="Type Your New Password" />
                                            <Input.Password name="confirm_new_password" onChange={onChange} size="large" placeholder="Confirm Your New Password" />
                                        </div>
                                        <div className="actions-change-password">
                                            <Button size="large" onClick={handleCancel} className="btn-save">취소</Button>
                                            <Button size="large" onClick={handleExitBtn} className="btn-save">나가기</Button>
                                        </div>
                                    </div>
                                </Modal>
                            </div>
